const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:4950';
...
const adminCookie = await login('admin@satellite-data-operations.local', 'admin123');
const managerCookie = await login('manager@satellite-data-operations.local', 'manager123');
const analystCookie = await login('analyst@satellite-data-operations.local', 'analyst123');
